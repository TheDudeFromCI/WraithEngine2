...
public class ChipsetImporter{
	...
	private void unwrap(){
		try{
			...
			if(width%WraithEngine.projectBitSize!=0||height%WraithEngine.projectBitSize!=0){
			...
			width /= WraithEngine.projectBitSize;
			height /= WraithEngine.projectBitSize;
			...
			for(x = 0; x<width; x++){
				for(y = 0; y<height; y++){
					...
					tileImages[index] = image.getSubimage(x*WraithEngine.projectBitSize, y*WraithEngine.projectBitSize, WraithEngine.projectBitSize,
						WraithEngine.projectBitSize);
